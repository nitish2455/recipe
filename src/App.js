...
<nav className="bg-blue-800 py-4 px-3">
        <div className="container mx-auto flex justify-between items-center">
          <div className='flex gap-4'>
          <h1 className="text-4xl font-bold text-white italic">Recipe Book</h1>
           <p className='text-xl pt-2 hidden sm:block text-white'>Home</p>
           <p className='text-xl pt-2  hidden sm:block text-white'>About</p>
          </div>
          <div className="">
            <button
      ...
      <p className='font-bold text-4xl italic py-4 text-center text-green-700'>Get Your Favourite Food Recipe Here</p>

      <section className="controls container mx-auto p-4">
        <p className='font-serif sm:text-3xl text-xl '>In this Section you can plan your meal according to your calories</p>
        <p className='font-semibold text-xl italic text-cyan-600 py-4'>Stay Healthy</p>
        <input
          type="number"
          placeholder="Calories (e.g. 2000)"
          onChange={handleChange}
          className='px-4 py-2 border border-gray-500 text-black'
        />
        <button
      ...
      <div className="grid gap-4 grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
        {recipes.map((recipe) => (
          <div
            key={recipe.id}
            className="border p-4 rounded-md cursor-pointer hover:shadow-2xl bg-cyan-900"
